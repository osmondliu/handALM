...
function check_form(formname) {
    if (typeof(siw) != 'undefined' && siw && typeof(siw.selectingSomething) != 'undefined' && siw.selectingSomething)
        ...
        $("#lineItems>tbody>tr").each(function(){
            $(this).children().each(function(){//td
                if($(this).children().is(".required")&&
                   $(this).next().children().attr("type")=="text"&&
                   $(this).next().children().val()==""){//存在必输字段并且是文本框且为空
                    //TODO::当已经出现提示文字后，不再出现
                    //if ($(this).next().has("div")) {}
                    var field=$(this).text().replace("*","");
                    var html='<div class="required validation-message">缺少必填字段:'+field+'</div>'; 
